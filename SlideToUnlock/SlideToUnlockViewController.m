...
- (void)loadView {
	...
	// Render the label text animation using our custom drawing code in the label's layer.
...
- (void)startTimer {
	if (!animationTimer) {
		...
		animationTimer = [NSTimer scheduledTimerWithTimeInterval:1.0/animationFramesPerSec
                                                          target:self
...
- (void)drawLayer:(CALayer *)theLayer inContext:(CGContextRef)theContext {
	...
	CGContextShowTextAtPoint(theContext, 0, (size_t)label.font.ascender, text, strlen(text));
